...
# Huang*****************************************************************************************
            ...
            secret_dir=r'/root/script/superset/secret.txt'
            ...
            # 清理脏数据
            with open(secret_dir,"r") as r:
                for line in r.readlines():
                    if ( len(str(line).rsplit("/",1)) != 2 ):
                        old_list.append(line.strip())
                    else :
                        new_list.append(line.strip())

            with open(secret_dir,"w") as w:
                for i in new_list:
                    w.write(i+"\n")
            ...
            # 创建两个个集合，一个存放密钥元数据文件，一个存放元数据生成的数字
            hash_set = set()
            input_set = set()
            ...
            with open(secret_dir,"r") as r:
                ...
                for line in r.readlines():
                    if ( len(str(line).rsplit("/",1)) != 2 ):
                        ...
                        line1 = "%s" %(str.strip(line.rsplit("/",1)[1]))
                        hash_set.add(line1)

            for value in hash_set:
                # 获取 secret_key 对应的一次性密码,并添加到 input_set 集合中
                input_set.add(pyotp.TOTP(value).now())

            token = form.mfa.data

            #判断token是否存在 input_set 集合中
